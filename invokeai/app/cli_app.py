...
from .services.default_graphs import (
...
def fields_from_type_hints(
    hints: dict[str, type], node_path: str
) -> dict[str, NodeField]:
    return {
        k: NodeField(alias=k, node_path=node_path, field=k, field_type=v)
        for k, v in hints.items()
    }


def get_node_input_field(
    graph: LibraryGraph, field_alias: str, node_id: str
) -> NodeField:
...
def get_node_inputs(
    invocation: BaseInvocation, context: CliContext
) -> dict[str, NodeField]:
    ...
    if node_type is not GraphInvocation:
        ...
        graph: LibraryGraph = context.invoker.services.graph_library.get(
            context.graph_nodes[invocation.id]
        )
        return {
            e.alias: get_node_input_field(graph, e.alias, invocation.id)
            for e in graph.exposed_inputs
        }
...
def get_node_outputs(
    ...
    node_type = type(invocation)
    if node_type is not GraphInvocation:
        return fields_from_type_hints(
            get_type_hints(node_type.get_output_type()), invocation.id
        )
    else:
        graph: LibraryGraph = context.invoker.services.graph_library.get(
            context.graph_nodes[invocation.id]
        ...
        return {
            e.alias: get_node_output_field(graph, e.alias, invocation.id)
            for e in graph.exposed_outputs
        }
...
def generate_matching_edges(
    ...
    matching_fields = [
        ...
        if are_connection_types_compatible(afields[f].field_type, bfields[f].field_type)
    ]
        ...
        try:
            ...
            for cmd in cmds:
                ...
                if args["type"] in system_graph_names:
                    system_graph = next(
                        filter(lambda g: g.name == args["type"], system_graphs)
                    )
                    invocation = GraphInvocation(
                        graph=system_graph.graph, id=str(current_id)
                    )
                ...
                if "link_node" in args and args["link_node"]:
                    for link in args["link_node"]:
                        ...
                        edges = [
                            e
                            for e in edges
                            if e.destination not in matching_destinations
                        ]
                        edges.extend(matching_edges)
                ...
                if "link" in args and args["link"]:
                    for link in args["link"]:
                        edges = [
                            e
                            for e in edges
                            if e.destination.node_id != command.command.id
                            or e.destination.field != link[2]
                        ]
                        ...
                        node_output = get_node_outputs(
                            context.session.graph.get_node(node_id), context
                        )[link[1]]
                        ...
                        edges.append(
                            Edge(
                                source=EdgeConnection(
                                    node_id=node_output.node_path,
                                    field=node_output.field,
                                ),
                                destination=EdgeConnection(
                                    node_id=node_input.node_path, field=node_input.field
                                ),
                            )
                        )
            ...
            invoker.services.logger.warning(
                'Invalid command arguments, run "<command> --help" for summary'
            )
