...
class ExtrudeDepthInvocation(BaseInvocation):
    ...
    shift: int = Field(20, description="Number of pixels to shift bottom from top")
    steps: int = Field(1000, description="Number of steps in gradient")
    ...
    def extrude(self, cv_mask: np.ndarray) -> np.ndarray:
        ...
        for frac in np.linspace(0, 1, self.steps):
            ...
            values = frac * cv_mask[mask_x : -canv_x - 1, mask_y : -canv_y - 1]
            sub_mask = values > canvas[canv_x : -mask_x - 1, canv_y : -mask_y - 1]
            canvas[canv_x : -mask_x - 1, canv_y : -mask_y - 1][sub_mask] = values[
                sub_mask
            ]
